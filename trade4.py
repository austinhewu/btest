...
def bollinger_bands_x2(df, window=147):
...
def trade4(df):
    df['position'] = 0  # long -> +1, short -> -1, max = 3
    df['trade_price_1'] = 0
    df['trade_price_2'] = 0
    df['trade_price_3'] = 0
    df['P/L'] = 0
    ...
    for i in range(1, len(df['close'])):
        ...
        elif df.loc[i - 1, 'position'] == 3:
            df.loc[i, 'P/L'] = df.loc[i, 'close'] * 3 - entry_1 - entry_2 - entry_3
            df.loc[i, '%P/L'] = (df.loc[i, 'P/L'] / (entry_1 + entry_2 + entry_3)) * 100
        ...
        if df.loc[i, '%P/L'] < -1 and df.loc[i - 1, 'position'] > 0:  # stop-loss for %P/L < -1
            ...
            pause = True
        if pause:
            if (df.loc[i, 'high'] > df.loc[i, 'upper_band_2SD'] and df.loc[i - 1, 'high'] <=
                    df.loc[i - 1, 'upper_band_2SD']):
            ...
            if (df.loc[i, 'high'] > df.loc[i, 'upper_band_2SD'] and df.loc[i - 1, 'high'] <=
                ...
                if df.loc[i - 1, 'position'] == 0:
                    ...
                    df.loc[i, 'position'] = df.loc[i - 1, 'position'] + 1
                    entry_3 = df.loc[i, 'close']
                    df.loc[i, 'trade_price_3'] = df.loc[i, 'close']
            elif (df.loc[i, 'low'] < df.loc[i, 'lower_band_2SD'] and df.loc[i - 1, 'low'] >=
                  df.loc[i - 1, 'lower_band_2SD']) or (df.loc[i, 'low'] < df.loc[i, 'lower_band_3SD'] and
                                                       df.loc[i - 1, 'low'] >= df.loc[i - 1, 'lower_band_3SD']):
...
data = trade4(data)
output_path = 'C:/Users/Austin/Downloads/BTEST/BTEST output 4.csv'
